...
export default function AchievementDetailSection() {
  ...
  if (!achievement) {
    return (
      <>
        <SEO title="Achievement Not Found" path={`/achievements/${id}`} />
  ...
  return (
    <>
      <SEO
        ...
        description={`${achievementTitle} — ${achievement.issuer}${achievement.date ? ` (${achievement.date})` : ''}. ${achievement.description || ''}`}
          ...
          <RevealWrapper>
            <GlassCard style={{ padding: '2.5rem' }}>
              <div className="achievement-card" style={{ padding: 0, animation: 'none', gap: '2rem' }}>
                ...
                <div style={{ flex: 1 }}>
                  <h1 className="achievement-title">{achievementTitle}</h1>
